...
apps:: classes/lib/libDwmMcCurtain.a
	${MAKE} -C apps

classes/lib/libDwmMcCurtain.a::
	${MAKE} -C classes

tarprep:: classes/lib/libDwmMcCurtain.a apps
	${MAKE} -C classes $@
	${MAKE} -C apps $@
ifeq ("${BUILD_DOCS}", "yes")
	${MAKE} -C doc $@
...
darwin-pkg: tarprep
	pkgbuild --root staging --identifier net.mcplex.mccurtain --version ${VERSION} --scripts etc/macos/scripts mccurtain-${VERSION}.pkg
...
distclean:: clean
	...
	rm -f mccurtain_*.deb mccurtain-*.pkg
